init python:
    ...
    def ingamebutton(text, clicked, previously=None):
        ...
        if previously:
            bgim = im.Composite(None,
                                    ...
                                    (1062,7), im.Alpha("ui/bt-cf-checked.png",0.5))
        elif previously is not None:
            bgim = im.Composite(None,
                                    (0,0), "ui/bg-choice.png",
                                    (1062,7), im.Alpha("ui/bt-cf-unchecked.png",0.5))
        else:
            bgim = "ui/bg-choice.png"
        
        bg = Fixed(bgim, xmaximum=1126)
        
        this_idle = LiveComposite((1126,65),
                                      (0,0), ingm_bg(bg),
                                      (0,0), ingm_btn(img)
                                      )
        this_hover = LiveComposite((1126,65),
                                      (0,0), ingm_bg(bg),
                                      (0,0), ingm_btn_hover(img)
                                      )
        
        ui.fixed(xpos=0.5, ymaximum=65, xmaximum=1920)
    ...
    def ingamebutton_dead(text, is_active=False):
        ...
        clicked = None
        if is_active:
            img = Fixed(Text(text, xalign=0.5, yalign=0.5, color="#ccc"), xmaximum=972)
            bg = Fixed("ui/bg-choice_nochoice.png", xmaximum=972)
        else:
            img = Fixed(Text(text, xalign=0.5, yalign=0.5, color="#00000080"), xmaximum=972)
            bg = Fixed(im.MatrixColor("ui/bg-choice.png",im.matrix.desaturate()*im.matrix.brightness(-0.2)*im.matrix.opacity(0.3)), xmaximum=972)
        
        ui.fixed(xpos=0.5, ymaximum=56, xmaximum=1920)
