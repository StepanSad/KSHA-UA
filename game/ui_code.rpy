...
init 2 python:
    ...
    def widget_button(text,displayable,clicked=None,style='prefs_label',xsize=220,ysize=60,widgetyoffset=3,textxoffset=30,state="button", xpos=0, ypos=0):
    ...
    def return_button(wherefunc, text=None, xpos=1242, ypos=810):
        if text == None:
            text = displayStrings.return_button_text
        
        ui.add(renpy.Keymap(game_menu=wherefunc))
        widget_button(text, "ui/bt-return.png", wherefunc, xsize=133, ysize=42, xpos=xpos, ypos=ypos, textxoffset=47, widgetyoffset=1)
    ...
    def custom_render_savefile(index, name, filename, extra_info, screenshot, mtime, newest, clickable, has_delete, **positions):
        ...
        ui.text(s, style=style.file_picker_extra_info[index], ypos=-6)
        ui.close()
        ui.vbox(xpos=-30)
        ui.null(height=27)
        if has_delete:
    ...
    def custom_file_picker(selected, save, mode="manual", background = None):
        ...
        while True:
            ...
            scrollable = False
            if len(displayed_slots) > 4:
                scrollable = True
            
            if scrollable:
                auto_offset = 1.0 / (len(displayed_slots) - 4)
            ...
            ui.window(style='file_picker_frame', xminimum=350)
            
            ui.vbox()
            
            if selected == "save":
                ui.text(displayStrings.save_page_caption, style="page_caption", xpos=-40)
            else:
                ui.text(displayStrings.load_page_caption, style="page_caption", xpos=-40)
            ui.null(height=15)
            ...
            def store_yoffset(input=persistent.fpicker_yoffset):
                global auto_offset, displayed_slots
                maxheight = auto_offset * (len(displayed_slots) - 4)
                if input > maxheight:
                    input = maxheight
                elif input < 0:
                    input = 0
            ...
            elif persistent.fpicker_yoffset > auto_offset * (len(displayed_slots) - 4) or persistent.fpicker_yoffset == -1:
                persistent.fpicker_yoffset = auto_offset * (len(displayed_slots) - 4)
            
            yadj = ui.adjustment(range=auto_offset * (len(displayed_slots) - 4), value=persistent.fpicker_yoffset, changed=store_yoffset)
            vp = ui.viewport(yadjustment=yadj, mousewheel=False, draggable=False, xmaximum=700, ymaximum = 500, xpos=-20, ypos = 3) 
            ui.vbox(xfill=False)
            ...
            ui.vbox(xpos = 1300, yalign=0.454, box_spacing=2)
            if scrollable:
            ...
            if selected == "save":
                ...
                widget_button(displayStrings.new_save_button, "ui/bt-star.png", ui.returns((str(nextsaveslot),False)), xsize=440, widgetyoffset=-7, textxoffset=53, xpos=580, ypos=810)
    ...
    def indicator_icons():
        ...
        ui.text(' '.join(res), xanchor=1.0, xpos=1899, ypos=10 )
    ...
    class TogglePreference(object):
        ...
        def render_preference(self, thisxpos=0, thisypos=0, disabled=False, widgetysize=50):
            ...
            if disabled:                                         
                widget_button(self.name, checkboximage, clicked, xsize=625, ysize=widgetysize, widgetyoffset=-8, textxoffset=40, state="disabled")
            else:
                widget_button(self.name, checkboximage, clicked, xsize=625, ysize=widgetysize, widgetyoffset=-8, textxoffset=40)
    ...
    class Gallery(object):
        ...
        def navigation(self, page_name, page_num, pages, currentpage=0):
            ...
            ui.hbox(background=None,xpos=617,ypos=850)
        ...
        def locked_images_screen(self, text):
            ...
            ui.add(self.locked_background)
            ui.text(text, xalign=0.5, yalign=0.5, style="prefs_label")
            ui.saybehavior()
            ui.interact()
        ...
        def show(self, page=0):
            ...
            while True:
                ...
                ui.text(displayStrings.gallery_page_caption, style="page_caption", xpos = 505, ypos = 198)
                if len(self.pages) > 1:
                ...
                return_button(_intra_jumps("extra_menu", "intra_transition"), xpos=1298, ypos=857)
        ...
        def autobutton(self, in_images):
            ...
            if isinstance(in_images, tuple):
                ...
                in_images = (in_images,)
            if not thumbnail:
                thumbnail = im.Scale(ImageReference(thumb_base), 100*2, 75*2)
            button_base = LiveComposite((110*2, 85*2),
                                            (0, 0), ib_base("ui/bt-cg-locked.png"),
                                            (10, 10), im.MatrixColor(thumbnail, im.matrix.desaturate()))
    ...
    def extra_button(text,in_displayable,clicked=None,style='prefs_label',state="button"):
        ...
        if state == "return": 
            textbase = LiveComposite((100, 30),
                                         (0, 1), ib_base("ui/bt-return.png"),
                                         (47, 0), textbase)
            texthover = LiveComposite((100, 30),
                                         (0, 1), "ui/bt-return.png",
                                         (47, 0), texthover)
    ...
    def toggle_widesay():
        persistent.widesay = not persistent.widesay
        return False
    ...
    def refresh_label(a,b):
        ...
        global ss_desc
        disp = Text(ss_desc, xalign=0.5, yalign=0.98, size=31, **displayStrings.styleoverrides)
        return (disp,0.05)
